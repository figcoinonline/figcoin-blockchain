...
const initHttpServer = (myHttpPort: number) => {
  ...
  app.get("/v2/blocksOffset", rateLimiter, (req, res) => {
    ...
    console.log(offset, page, "offset, page");

    if (!(offset > 0)) {
      return res.status(400).send("invalid_offest");
    }

    if (page > BLOCKCHAIN_CHUNK_SIZE) {
      return res.status(400).send("invalid_page_size");
    }

    res.status(200).send(getBlockchainWithOffset(offset, page));
  });

  app.get("/v2/block/:hash", rateLimiter, (req, res) => {
    const blocks = searchBlockchain(
      (blk) => blk.hash === req.params.hash,
      true
    );
  ...
  app.post("/v2/figTransaction", rateLimiter, (req, res) => {
    ...
    try {
      const resp = generateNextBlockWithTransaction(
        address,
        amount,
        nodeSecret
      );
  ...
  app.post("/v2/sendFromWalletTransactionToPool", rateLimiter, (req, res) => {
    try {
      ...
      const resp = sendFromWalletTransactionToPool(
        publicKey,
        privateKey,
        toPublicKey,
        amount
      ...
      if (newBlock === null || newBlock === undefined) {
        if (resp) {
          let returnObject = {
            success: true,
            msg: "New Transaction Send To Pool",
            data: resp,
          };
          res.status(200).send(returnObject);
        } else {
          let returnObject = {
            success: false,
            msg: "Error when save Transaction.",
          };
